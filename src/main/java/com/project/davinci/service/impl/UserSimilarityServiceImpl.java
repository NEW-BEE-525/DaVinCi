...
import com.project.davinci.persistence.UserSimilarityDAO;
...
public class UserSimilarityServiceImpl implements UserSimilarityServive {
    ...
    public boolean saveUserSimilarity(UserSimilarity userSimilarity) {
        ...
        int rows = this.userSimilarityDAO.saveUserSimilarity(userSimilarity);
        if (rows > 0) {
            flag = true;
        }
        return flag;
    ...
    public boolean updateUserSimilarity(UserSimilarity userSimilarity) {
        ...
        int rows = this.userSimilarityDAO.updateUserSimilarity(userSimilarity);
        if (rows > 0) {
            flag = true;
        }
        return flag;
    ...
    public boolean isExistsUserSimilarity(UserSimilarity userSimilarity) {
        int count = this.userSimilarityDAO.countUserSimilarity(userSimilarity);
        if (count > 0) {
            return true;
        }
        return false;
    ...
    public List<UserSimilarity> listUserSimilarityByUId(Long userId) {
        ...
        List<UserSimilarity> userSimilarityList = this.userSimilarityDAO.listUserSimilarityByUId(userId);
