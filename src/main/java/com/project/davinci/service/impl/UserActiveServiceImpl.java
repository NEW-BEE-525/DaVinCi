...
import com.project.davinci.persistence.UserActiveMapper;
...
public class UserActiveServiceImpl implements UserActiveService {
    ...
    public boolean saveUserActive(UserActive userActive) {
        ...
        int rows = this.userActiveMapper.countUserActive(userActive);
        int saveRows = 0;
        int updateRows = 0;
        // 2.不存在就添加
        if (rows < 1) { // 不存在
            userActive.setHits(1L); // 不存在记录的话那肯定是第一次访问，那点击量肯定是1
            saveRows = this.userActiveMapper.saveUserActive(userActive);
        } else { // 已经存在
            // 3.存在则先把当前用户对当前二级类目的点击量取出来+1
            long hits = this.userActiveMapper.getHitsByUserActiveInfo(userActive);
            // 4.然后在更新用户的浏览记录
            hits++;
            userActive.setHits(hits);
            updateRows = this.userActiveMapper.updateUserActive(userActive);
        }
